...
* 4つのリリース期間外のリリースからアップグレードする場合は、複数ステップのアップグレードを実行してください。アップグレード元のバージョンに対応するアップグレード手順に従って、4つのリリース期間内に収まる最新リリースにアップグレードしてください。例えば、22.01 を実行していて、24.06 にアップグレードしたい場合（：）
+
...
一般的には、最初のインストールに使用したのと同じ方法でアップグレードする必要がありますが、link:../trident-install/kubernetes-deploy.html#moving-between-installation-methods["インストール方法を切り替えます"]。Trident をアップグレードするには 2 つのオプションがあります。
