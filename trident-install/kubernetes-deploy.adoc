...
TIP: link:../trident-get-started/intro.html["基本概念"^]にまだ慣れていない場合は、今こそそれを実行する絶好の機会です。
...
手動でのデプロイでもHelmを使用する場合でも、Tridentオペレーターはインストールを簡素化し、Tridentリソースを動的に管理する優れた方法です。link:../trident-install/kubernetes-customize-deploy.html["Tridentのオペレータ環境をカスタマイズ"] を使用して、 `TridentOrchestrator` カスタムリソース（CR）の属性を設定することもできます。
