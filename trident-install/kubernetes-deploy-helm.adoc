...
.開始する前に
link:../trident-install/kubernetes-deploy.html#before-you-deploy["導入の前提条件"]に加えてlink:https://v3.helm.sh/["Helm バージョン 3"^]が必要です。
...
----
. を使用し `helm install`、次の例のように導入環境の名前を指定します。 `100.2410.0`は、インストールするTridentのバージョンです。
+
[listing]
----
