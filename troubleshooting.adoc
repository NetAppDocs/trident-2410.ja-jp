...
* Tridentを https://azure.microsoft.com/en-us/products/netapp/["Azure NetApp Files"]で使用する際に発生する可能性のある一般的な問題は、テナントシークレットとクライアントシークレットが、権限が不十分なアプリ登録から取得された場合です。Trident要件の完全なリストについては、link:trident-use/anf.html["Azure NetApp Files"]構成を参照してください。
