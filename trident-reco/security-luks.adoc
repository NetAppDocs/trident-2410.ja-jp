...
----
. LUKS暗号化を使用してストレージプールを定義する場合に使用し `parameters.selector`ます。例：
+
[listing]
----
...
.手順
. および `node-publish-secret-namespace`StorageClassパラメータを追加します `node-publish-secret-name`。例：
+
[listing]
----
apiVersion: storage.k8s.io/v1
...
. および `node-expand-secret-namespace`StorageClassパラメータを追加します `node-expand-secret-name`。例：
+
[listing]
----
apiVersion: storage.k8s.io/v1
