...
|===
[verse]
上記の表の脚注: Yesfootnote:1[]: Tridentで管理されていないYesfootnote:2[]: Tridentで管理されているがPVではないYesfootnote:3[]: Tridentで管理されていないPVで管理されていないYesfootnote:4[]: rawブロックボリュームでサポートYesfootnote:5[]: Tridentでサポート
...
NOTE: Kubernetes のツリー内プロビジョニングツールは、現時点では NFS または iSCSI PVS のボリュームサイズ変更処理をサポートしていません。Trident では、 NFS ボリュームと iSCSI ボリュームの拡張がサポートされています。
